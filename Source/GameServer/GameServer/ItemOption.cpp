...
void CItemOption::Load(char* path) // OK
{
	...
	try
	{
		while(true)
		{
			...
			auto it = this->m_ItemOptionInfo.find(info.Index);

			if(it == this->m_ItemOptionInfo.end())
			{
				this->m_ItemOptionInfo[info.Index] = std::vector<ITEM_OPTION_INFO>(1, info);
			}
			else
			{
...
bool CItemOption::GetItemOption(int index,CItem* lpItem) // OK
{
	...
	auto ItemOptionInfo = this->m_ItemOptionInfo.find(index);
	...
	for(int n=0;n < MAX_SPECIAL;n++)
	{
		...
		switch(index)
		{
			...
			case ITEM_OPTION_ADD_CURSE_DAMAGE_BY_LEVEL:
				lpObj->CurseDamageMin += (lpObj->Level/value);
				lpObj->CurseDamageMax += (lpObj->Level/value);
				break;
			...
			case ITEM_OPTION_ADD_HP_BY_LEVEL:
				lpObj->AddLife += (lpObj->Level/value);
				break;
			case ITEM_OPTION_ADD_MP_BY_LEVEL:
				lpObj->AddMana += (lpObj->Level/value);
				break;
			...
			case ITEM_OPTION_ADD_DEFENSE_BY_LEVEL:
				lpObj->Defense += (lpObj->Level/value);
				break;
