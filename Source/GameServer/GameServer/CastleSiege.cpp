...
int CCastleSiege::LoadData(char * lpszFileName)
{
	...
	std::unordered_map<UINT,int> mpNpcDataCheck;
	...
	while( true )
	{
		...
		while ( true )
		{
			...
			else if(type == 2)
			{
				...
				auto it = mpNpcDataCheck.find(uNpcKey);
				...
				auto index = MACRO2(iNPC_INDEX) | (MACRO2(iNPC_NUM) << 16);
				mpNpcDataCheck[index] = 0;
...
void CCastleSiege::SetCalcRegGuildList(CSP_CALCREGGUILDLIST* lpMsg, int iCOUNT)
{
	...
	for(int iGCNT = 0; iGCNT < iGUILD_COUNT; iGCNT++ )
	{
		...
		this->m_mapCsBasicGuildInfo[vtRegGuildScore[iGCNT].m_strGuildName] = GuildData;
	}
	...
	if( this->m_btIsCastleOccupied == TRUE )
	{
		if( strcmp(this->m_szCastleOwnerGuild, "") != 0 )
		{
			...
			this->m_mapCsBasicGuildInfo[m_szCastleOwnerGuild] = GuildData;
		}
	}
	...
	auto it = this->m_mapCsBasicGuildInfo.begin();
	...
	for(; it != this->m_mapCsBasicGuildInfo.end(); it++)
	{
		...
		this->m_mapCsTotalGuildInfo[it->first] = it->second;
...
void CCastleSiege::MakeCsTotalGuildInfo(CSP_CSGUILDUNIONINFO* lpMsg, int iCOUNT)
{
	...
	for( int iGCNT = 0; iGCNT < iCOUNT; iGCNT++ )
	{
		...
		auto it = this->m_mapCsTotalGuildInfo.find(szGuildName);
		...
		this->m_mapCsTotalGuildInfo[szGuildName] = GuildData;
	}
...
void CCastleSiege::SaveCsTotalGuildInfo()
{
	...
	auto it = this->m_mapCsTotalGuildInfo.begin();
	...
	auto it2 = this->m_mapCsTotalGuildInfo.begin();
...
void CCastleSiege::SetCsTotalGuildInfo(CSP_CSLOADTOTALGUILDINFO* lpMsg, int iCOUNT)
{
	...
	for( int iGCNT = 0; iGCNT < iCOUNT; iGCNT++ )
	{
		...
		{
			EnterCriticalSection(&this->m_critCsBasicGuildInfo);
			this->m_mapCsBasicGuildInfo[szGuildName] = GuildData;
			LeaveCriticalSection(&this->m_critCsBasicGuildInfo);
		}
	}
...
int CCastleSiege::GetCsJoinSide(char* lpszGuildName, BYTE * btCsJoinSide, bool * bCsGuildInvolved)
{
	...
	*bCsGuildInvolved = 0;
	...
	auto it = this->m_mapCsTotalGuildInfo.find(lpszGuildName);
...
void CCastleSiege::ChangeWinnerGuild(int iCsJoinSide)
{
	...
	for( int iUSER = OBJECT_START_USER; iUSER < MAX_OBJECT; iUSER++ )
	{
		...
		auto it = this->m_mapCsTotalGuildInfo.find(szNewGuild);
...
void CCastleSiege::OperateMiniMapWork()
{
	...
	std::unordered_map<int, _CS_MINIMAP_DATA> mapMiniMapData;
	...
	for(auto it = this->m_mapCsBasicGuildInfo.begin();  it != this->m_mapCsBasicGuildInfo.end(); it++)
	{
		...
		mapMiniMapData[GuildData->m_iCsGuildID] = MiniMapData;
	}
	...
	auto it2 = mapMiniMapData.begin();
	
	for(; it2 != mapMiniMapData.end(); it2++ )
	{
		this->m_mapMiniMapData[it2->first] = it2->second;
	}
	...
	for( int iIDX = OBJECT_START_USER; iIDX < MAX_OBJECT; iIDX++ )
	{
		...
		auto it3 = this->m_mapMiniMapData.find(gObj[iIDX].CsJoinSide);
	...
	auto it4 = this->m_mapMiniMapData.begin();
