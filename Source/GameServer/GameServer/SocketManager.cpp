		...
		if (size <= lpIoBuffer->size)
		{
			if (header == 0xC3 || header == 0xC4)
			{
				if (header == 0xC3)
				{
					...
					if (OBJECT_RANGE(index) != 0 && gObj[index].Connected != OBJECT_OFFLINE)
					{
						ProtocolCore(head, DecBuff, DecSize, index, 1, DecSerial);
					}
				}
				else
				{
			...
			else
			{
				...
				if (OBJECT_RANGE(index) != 0 && gObj[index].Connected != OBJECT_OFFLINE)
				{
					ProtocolCore(head, DecBuff, size, index, 0, -1);
				}
			}
