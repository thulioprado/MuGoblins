		...
		while (true)
		{
			...
			while (true)
			{
				...
				else if (section == 4)
				{
					...
					auto it = this->m_ItemBagInfo.find(info.Index);
				...
				else if (section >= 5)
				{
					...
					{
						this->m_ItemBagItemInfo[section] = std::vector<ITEM_BAG_EX_ITEM_INFO>(1, info);
					}
					else
					{
...
{
	for (auto it = this->m_ItemBagInfo.begin(); it != this->m_ItemBagInfo.end(); it++)
	{
		if ((GetLargeRand() % 10000) < it->second.DropRate)
		{
			...
			auto ItemInfo = this->m_ItemBagItemInfo.find(lpItemBagDropInfo->Section);
			...
			lpItem->Convert(ItemIndex, ItemOption1, ItemOption2, ItemOption3, ((ItemNewOption == 0) ? lpItemBagItemInfo->Grade : ItemNewOption), ItemSetOption);
