...
void CMove::Load(char* path) // OK
{
	...
	try
	{
		while(true)
		{
			...
			this->m_MoveInfo[info.Index] = info;
		}
	}
	catch(...)
	{
...
{
	auto it = this->m_MoveInfo.find(index);
...
{
	for(auto it=this->m_MoveInfo.begin();it != this->m_MoveInfo.end();it++)
	{
...
void CMove::CGTeleportRecv(PMSG_TELEPORT_RECV* lpMsg,int aIndex) // OK
{
	...
	if(lpMsg->gate == 0)
	{
		...
		if(lpSkill != 0)
		{
			...
			gSkillManager.GCSkillAttackSend(lpObj,lpSkill->m_skill,aIndex,1);
