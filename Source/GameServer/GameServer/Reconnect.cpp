...
{
	for(auto it=this->m_ReconnectInfo.begin();it != this->m_ReconnectInfo.end();)
	{
...
void CReconnect::SetReconnectInfo(LPOBJ lpObj) // OK
{
	...
	auto it = this->m_ReconnectInfo.find(std::string(lpObj->Name));

	if(it == this->m_ReconnectInfo.end())
	{
		this->m_ReconnectInfo[lpObj->Name] = info;
	}
	else
	{
		it->second = info;
...
bool CReconnect::CheckResumeParty(RECONNECT_INFO* lpInfo) // OK
{
	...
	{
		for(auto it=this->m_ReconnectInfo.begin();it != this->m_ReconnectInfo.end();it++)
		{
