...
void gObjSetExperienceTable() // OK
{
	...
	for (int i = 1, x = 1, a = 1, b = 1, c = 1, d = 1, e = 1; i <= MAX_CHARACTER_LEVEL; ++i)
	{
		gLevelExperience[i] = (((QWORD(i) + 9) * i) * i) * 10;

		if (i >= 100)
		{
			gLevelExperience[i] += (((QWORD(x) + 9) * x) * x) * 100;
			++x;
		}

		if (i >= 1000)
		{
			gLevelExperience[i] += (((QWORD(a) + 9) * a) * a) * 1000;
			++a;
		}

		if (i >= 3000)
		{
			gLevelExperience[i] += (((QWORD(b) + 9) * b) * b) * 10000;
			++b;
		}

		if (i >= 5000)
		{
			gLevelExperience[i] += (((QWORD(c) + 9) * c) * c) * 100000;
			++c;
		}

		if (i >= 7000)
		{
			gLevelExperience[i] += (((QWORD(d) + 9) * d) * d) * 1000000;
			++d;
		}

		if (i >= 9000)
		{
			gLevelExperience[i] += (((QWORD(e) + 9) * e) * e) * 10000000;
			++e;
		}
	}
}
...
void gObjSecondProc()
{
	...
	for (n = 0; n < MAX_OBJECT; ++n)
	{
		...
		if (lpObj->Connected > OBJECT_LOGGED)
		{
			...
			{
				if ((tick - lpObj->MapServerMoveQuitTickCount) > 30000)
				{
					gObjDel(lpObj->Index);
					continue;
				}
			...
			if (lpObj->Type == OBJECT_MONSTER)
			{
				...
				if (lpObj->MonsterDeleteTime != 0 && tick >= lpObj->MonsterDeleteTime)
				{
					gObjDel(lpObj->Index);
					continue;
				}
		...
		{
			if ((tick - lpObj->AutoSaveTime) > 600000)
			{
				GDCharacterInfoSaveSend(lpObj->Index);
				lpObj->AutoSaveTime = tick;

				gWarehouse.GDWarehouseInfoSend(lpObj->Index, lpObj->Account);
			}

			if (lpObj->CheckSumTime > 0 && (tick - lpObj->CheckSumTime) > 5000)
			{
		...
		if (lpObj->Connected == OBJECT_CONNECTED || lpObj->Connected == OBJECT_LOGGED || lpObj->Connected == OBJECT_ONLINE)
		{
			if (lpObj->Type == OBJECT_USER)
			{
				if (lpObj->Connected >= OBJECT_LOGGED)
				{
					if ((tick - lpObj->ConnectTickCount) > 60000)
					{
						CloseClient(n);
				...
				{
					if ((tick - lpObj->ConnectTickCount) > 30000)
					{
						CloseClient(n);
...
void gObjDelayLifeCheck(int aIndex)
{
	...
	if (lpObj->CheckLifeTime > 0)
	{
		...
		if (lpObj->CheckLifeTime <= 0)
		{
			...
			{
				gObjectManager.CharacterLifeCheck(lpObj->AttackObj, lpObj, 0, 1, 0, 0, 0);
			}
		}
	}
}
