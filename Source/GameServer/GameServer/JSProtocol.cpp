...
void JoinServerProtocolCore(BYTE head, BYTE* lpMsg, int size) // OK
{
	switch (head)
	{
		...
		case 0x30:
			...
			break;
		case 0x31:
			JGTotalUpdateRecv((SDHP_JOIN_SERVER_TOTAL_RECV*)lpMsg);
			break;
		case 0x32:
			JGPostMessageRecv((SDHP_POST_MESSAGE_RECV*)lpMsg);
			break;
	}
}
...
{
	GCDiscordUpdate(lpMsg->total);
}

void JGPostMessageRecv(SDHP_POST_MESSAGE_RECV* lpMsg)
{
	GCPostMessageSend(lpMsg->message);
}
