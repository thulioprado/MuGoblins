...
void CShopManager::Load(char* path) // OK
{
	...
	try
	{
		while(true)
		{
			...
			this->m_ShopManagerInfo[info.Index] = info;
		}
	}
	catch(...)
	{
...
{
	std::unordered_map<int,int> LoadShop;
	...
	do
	{
		if((data.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) == 0)
		{
			if(isdigit(data.cFileName[0]) != 0 && isdigit(data.cFileName[1]) != 0 && isdigit(data.cFileName[2]) != 0)
			{
				...
				{
					auto it = this->m_ShopManagerInfo.find(atoi(data.cFileName));
...
{
	for(auto it=this->m_ShopManagerInfo.begin();it != this->m_ShopManagerInfo.end();it++)
	{
...
{
	auto it = this->m_ShopManagerInfo.find(index);
