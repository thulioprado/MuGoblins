...
void CMapServerManager::Load(char* path)
{
		...
		while(true)
		{
			...
			while(true)
			{
				if(section == 0)
				{
					...
					this->m_MapServerCode[sSVR_CODE] = lpMapSvrData;
				...
				else if(section == 1)
				{
					...
					auto it = this->m_MapServerCode.find(sSVR_CODE);
	...
	auto it = this->m_MapServerCode.find(gServerInfo.m_ServerCode);
...
bool CMapServerManager::GetMapServerData(WORD ServerCode,char* IpAddress,WORD* ServerPort) // OK
{
	...
	auto it = this->m_MapServerCode.find(ServerCode);
	...
	switch ( sMAP_MOVE_INFO )
	{
		case -1:
			{
				...
				for ( auto it = this->m_MapServerCode.begin() ; it != this->m_MapServerCode.end() ;it++)
				{
		...
		default:
			if ( sMAP_MOVE_INFO > -1 )
			{
				...
				auto it = this->m_MapServerCode.find(sMAP_MOVE_INFO);
