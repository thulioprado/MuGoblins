...
{
	if(map != MAP_LORENCIA && map != MAP_DUNGEON && map != MAP_DEVIAS && map != MAP_NORIA && map != MAP_LOST_TOWER && map != MAP_ATLANS && map != MAP_TARKAN && map != MAP_AIDA && map != MAP_CRYWOLF)
	{
		return;
	}
...
bool CMap::MonsterItemDrop(int index,int level,float dur,int x,int y,BYTE Option1,BYTE Option2,BYTE Option3,BYTE NewOption,BYTE SetOption,int aIndex,DWORD serial,DWORD duration) // OK
{
	if(this->CheckAttr(x,y,4) != 0 || this->CheckAttr(x,y,8) != 0)
	{
		return 0;
	...
	for(int n=0;n < MAX_MAP_ITEM;n++)
	{
		if(this->m_Item[count].IsItem() == 0)
		{
			this->m_Item[count].CreateItem(index,level,x,y,dur,Option1,Option2,Option3,NewOption,SetOption,serial,duration);
...
bool CMap::ItemDrop(int index,int level,float dur,int x,int y,BYTE Option1,BYTE Option2,BYTE Option3,BYTE NewOption,BYTE SetOption,DWORD serial,int aIndex,int PetLevel,int PetExp,DWORD duration) // OK
{
	if(this->CheckAttr(x,y,4) != 0 || this->CheckAttr(x,y,8) != 0)
	{
		return 0;
	...
	for(int n=0;n < MAX_MAP_ITEM;n++)
	{
		if(this->m_Item[count].IsItem() == 0)
		{
			this->m_Item[count].DropCreateItem(index,level,x,y,dur,Option1,Option2,Option3,NewOption,SetOption,serial,PetLevel,PetExp,duration);
			this->m_Item[count].m_UserIndex = aIndex;
			this->m_ItemCount = (((++this->m_ItemCount)>=MAX_MAP_ITEM)?0:this->m_ItemCount);
