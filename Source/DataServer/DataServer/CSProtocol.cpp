...
void FriendMemoSend(FHP_FRIEND_MEMO_SEND* lpMsg,int index)
{
	...
	gQueryManager.BindParameterAsString(2, lpMsg->ToName, sizeof(lpMsg->ToName));
	gQueryManager.BindParameterAsString(3, lpMsg->Subject, sizeof(lpMsg->Subject));
	gQueryManager.ExecQuery("WZ_WriteMail ?,?,?,%d,%d",lpMsg->Dir,lpMsg->Action);
	gQueryManager.Fetch();
